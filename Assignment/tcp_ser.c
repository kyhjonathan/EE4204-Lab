...
***********************************/
//if ack.num == 1, means ACK, if ack.num == 0 means NAK
...
void str_ser(int sockfd)
{
	...
	while (!end) {
		...
		int random = rand() % 100;
		// printf("%d\n", random);
		if (random < ERRORPROB * 100) { // simulates packet loss at ERRORPROB, then sends NACK
			ack.num = 0; // NACK
			ack.len = 0;
			printf("Packet loss simulated, NACK sent\n");
		} else {
			ack.num = 1; // ACK
			ack.len = 0;
		...
		if ((n = send(sockfd, &ack, sizeof(ack), 0)) == -1) {
			perror("Error sending ACK!"); // send the ack
			exit(1);
		}
	}
