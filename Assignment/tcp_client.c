...
float str_cli(FILE *fp, int sockfd, long *len)
{
  ...
  /*** the whole file is loaded in the buffer. ***/
	...
	while(ci<= lsize)
	{
        ...
        while (ack.num == 0) { //while NACK is received, resend data
            printf("NACK received, resending...\n");
            n = send(sockfd, &sends, slen, 0);
            if(n == -1) {
                perror("send error!");								//send the data
                exit(1);
            }
            n = recv(sockfd, &ack, 2, 0); //receive the ACK or NACK
            if (n == -1) {
                perror("error when receiving ACK or NACK\n");
                exit(1);
            }
        }
        ci += slen;
